...
class Turret:
  ...
  def __init__(self, dic, log):
    ...
    # thing is deployed. Pause is a delay after movement. These work for me and mine.
    self.minx = 30
    self.maxx = 150
    self.miny = 50
    self.maxy = 180
  ...
  def tilt_to(self, angle, opts={}):
    ...
    print(f'pan_to {angle}, from {self.pan_angle}')
    meth = opts.get('method', Move.direct)
    pause = opts.get('pause',self.pause)
    incr = opts.get('increment', 10)
    if meth == Move.time:
      incr = opts.get('deadline',1)
    if self.state == State.stopping:
...
if __name__ == '__main__':
  # oins are GPIO, aka BCM. Not board.
  tdict = {"hw": True, 'laser_pin': 17, 'pan_pin': 0, 'tilt_pin': 1}
  t = Turret(tdict, None)
  # start Centered
  t.begin()
  t.pan_to(90)

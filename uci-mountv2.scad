...
module mount() {
    ...
    translate([-10,0,-5]) difference () {
      camera_mount();
      // hole for diode wires
      translate([-8,13,4]) rotate([0,90,0]) cylinder(r=6,h=35,$fn=50);
    }
    // diode barrel
    translate([9,15,-1]) rotate([0,90,0]) 
      difference() {
...
module camera_mount() {
  difference() {
    union() {
      translate([0,4,-5]) rotate([90,0,90]) cube([20,20,20]);
      translate([0,24,15]) rotate([90,90,90]) union() {
        difference() {
          cube([50,5,20]);
          translate([38,8,11]) rotate([90,0,0]) cylinder(r=4,h=10);
        }
